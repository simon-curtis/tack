...
/// Single-quotes `text` for POSIX shell command strings.
...
/// The Windows PowerShell-style command briefly written by the installer.
fn powershell_hook_command(exe_path: &Path) -> String {
    format!("& \"{}\" hook pre-tool-use", exe_path.display())
}
...
mod tests {
    ...
    #[test]
    fn merge_hook_entry_replaces_powershell_command() {
        let exe = fake_exe();
        let powershell_command = powershell_hook_command(&exe);
        let mut value = serde_json::json!({
            "hooks": {
                "PreToolUse": [
                    {
        ...
        let arr = value["hooks"]["PreToolUse"].as_array().expect("array");
        assert!(changed);
        assert_eq!(arr.len(), 1, "PowerShell entry must not be duplicated");
        assert_eq!(arr[0]["hooks"][0]["command"], our_hook_command(&exe));
